...
import { IAddDepartmentFormValues, IReactSelectInterface, IAddTimeFormValues } from "../../../interfaces";
import { toast } from "react-toastify";
import { AddDepartmentValidationSchema, AddTimeValidationSchema } from "../../../validations";
...
const Departments: FunctionComponent<RouteComponentProps> = (props: any) => {
  ...
  const [getDepartmentList, { data: departmentList, refetch }] = useLazyQuery<any>(GET_DEPARTMENT_LIST);
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (isActive > -1) {
        ...
        toast.success(languageTranslation('UPDATE_DEPARTMENT_CARE_INSTITUTION'));
      } else {
        await addDivision({
          variables: {
            id: parseInt(Id),
  ...
  const onDelete = async (id: string) => {
    ...
    if (!value) {
      ...
      if (!toast.isActive(toastId)) {
        toastId = toast.success(languageTranslation("DEPARTMENT_DELETE_SUCCESS_MSG"));
      }
    }
  };
                        ...
                        <div className="common-list-body border-0">
                          <ul className="common-list list-unstyled">
                            {departmentList && departmentList.getDivision.length ? (
                              departmentList.getDivision.map((item: any, index: number) => {
                                return (
                                  <li key={index} className={"cursor-pointer " + (isActive === index ? "active" : null)}
                                    onClick={() => {
