...
const CaregiverFormView: FunctionComponent<FormikProps<ICaregiverFormValue> &
  ...
  any> = (
    ...
    console.log("workingHoursFromDate", workingHoursFromDate);

    // Find difference in workingHours date
    const workingHourDateValidator = () => {
      let current = moment(new Date()).add(1, 'd').format(defaultDateFormat);
      // console.log("Current Date", current, dateData);
      const workingHoursDate = moment(workingHoursFromDate).format(defaultDateFormat)
      console.log(">>>>>>>>>>>>>",workingHoursDate);
      let dateFrom = moment(workingHoursDate).isBetween(dateData, current)
                                    ...
                                    <Field name={'workingHoursFromTime'}>
                                      {({ field }: any) => (
                                        <MaskedInput
                                          ...
                                          value={workingHoursFromTime ? workingHoursFromTime : ''}
                                        />
                                      )}
                                    </Field>
                                    {errors.workingHoursFromTime && touched.workingHoursFromTime && (
                                    ...
                                    <Field name={'workingHoursToTime'}>
                                      {({ field }: any) => (
                                        <MaskedInput
                                          ...
                                          onBlur={handleBlur}
                                          placeholder={languageTranslation("TIME_FORMAT")}
                                          value={workingHoursToTime ? workingHoursToTime : ''}
                                        />
                                      )}
                                    </Field>
